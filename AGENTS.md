...
6. **El área de staging (`.git/index`) es global del repositorio, no del proceso.** Norma incorporada el 2026-05-24 tras dos incidentes reales reseteados (commits míos que debían tocar `ROADMAP.md` o `AGENTS.md` aterrizaron con 12 archivos untracked de la sesión paralela del caso Lezo). **Causa real diagnosticada**: cuando dos sesiones paralelas trabajan sobre el mismo working tree, ambas escriben sobre el mismo `.git/index`. Si la sesión paralela ejecuta su `git add` entre tu `git status -s` de verificación y tu `git commit` siguiente, tu commit recoge **todo** el contenido del index global en ese instante, incluido lo que la otra sesión acaba de stagear. La ventana de carrera existe en cualquier sucesión `git add ... → git status -s → git commit` aunque los comandos estén en llamadas separadas, porque `git status -s` solo dice "esto había en este instante" pero no congela el index para los procesos vecinos. Inicialmente el incidente se atribuyó al uso de `&&` para encadenar `git add` y `git commit`, pero el diagnóstico tras un segundo incidente con comandos separados demostró que el problema es la concurrencia sobre el `.git/index` global, no el operador `&&`. La regla del punto 3 (`git status -s` antes del commit) sigue siendo útil para detectar lo ya commiteado por otro o lo ya staged en ese instante, pero **no garantiza nada sobre el momento del siguiente `git commit`**. Recuperación estándar si el incidente ocurre: `git reset HEAD~1` no destructivo (los archivos ajenos vuelven al working dir como untracked, intactos) y, antes de hacerlo, **comprobar con `git log` y `git reflog` qué commit está en HEAD** — si la sesión paralela ha commiteado entremedias, `HEAD~1` puede ser el commit de ELLA y no el tuyo, y un reset ciego revertiría su trabajo en lugar del tuyo.
7. **No hagas commits intermedios cuando hay sesiones paralelas activas.** Norma incorporada el 2026-05-24 por feedback del maintainer ("hasta que no digamos cerramos sesión no se hace commit ni git add"). Es la **única mitigación efectiva** del race del punto 6. Cada `git add` o `git commit` durante la sesión es una ventana de potencial contaminación cruzada con el staging de las otras sesiones. **No hagas `git add` ni `git commit` durante la sesión**: edita los archivos en el working tree, acumula todos los cambios, y commitea sólo cuando el maintainer indique explícitamente "cerramos sesión" (o equivalente). Esto refuerza la norma existente de §"Granularidad de commits" desde el ángulo multiagéntico: una sesión con N bloques temáticos commitea idealmente 1 commit (no N) y al final, salvo que la mezcla de naturalezas obligue a separar (ver excepción legítima en §"Granularidad de commits"). Aplica con especial cuidado a actualizaciones de `ROADMAP.md`: si vas a actualizarlo varias veces durante la sesión (cierre de PR, aprendizaje, lección incidente, etc.), espera al final y junta las ediciones en un solo commit ROADMAP final.
8. **Solución estructural a futuro: `git worktree`.** El race del punto 6 desaparece si cada sesión paralela trabaja sobre su propio working tree con su propio `.git/index` aislado, vía `git worktree add ../presuntamente-sesion-X main`. Cada worktree apunta al mismo `.git/` pero tiene su área de staging independiente. Los commits se aplican secuencialmente sobre el repo común. **Pendiente**: documentar el flujo de `git worktree` para sesiones paralelas si la frecuencia de incidentes lo justifica. Mientras tanto, la norma 7 (commit sólo al cierre de sesión) es suficiente operativamente.

...
6. **NUNCA encadenes `git add` y `git commit` con `&&` en un único comando.** Norma reforzada el 2026-05-24 tras un segundo incidente real: un commit que debía tocar sólo `ROADMAP.md` arrastró 12 archivos untracked de la sesión paralela del caso Lezo. Hipótesis más plausible: condición de carrera en el área de staging — entre el `git add ROADMAP.md` y el `git commit` siguiente del mismo comando combinado con `&&`, la sesión paralela hizo su propio `git add content/casos/lezo/...` y `git commit` recogió todo lo staged en ese instante. La regla del punto 3 (`git status -s` antes del commit) sólo cumple su propósito si las tres operaciones `git add` · `git status -s` · `git commit` se ejecutan como llamadas discretas que el agente pueda inspeccionar entre paso y paso. Combinarlas con `&&` colapsa la ventana de verificación a cero. Recuperación estándar si el incidente ocurre: `git reset HEAD~1` no destructivo (los archivos ajenos vuelven a `??` en el working dir, intactos) + rehacer el commit con `git add` por ruta explícita y `git status -s` entre medias.
7. **No hagas commits intermedios cuando hay sesiones paralelas activas.** Norma incorporada el 2026-05-24 por feedback del maintainer ("al estar trabajando multiagente nos tenemos que evitar esto"). Cada commit es una ventana de potencial contaminación cruzada con el staging de las otras sesiones. **Acumula todos los cambios de la sesión en el menor número posible de commits finales**, y nunca commitees bloques internos sueltos que podrías juntar. Esto refuerza la norma existente de §"Granularidad de commits" desde el ángulo multiagéntico: una sesión con N bloques temáticos commitea idealmente 1 commit (no N), salvo que la mezcla de naturalezas obligue a separar (ver excepción legítima en §"Granularidad de commits"). Aplica con especial cuidado a actualizaciones de `ROADMAP.md`: si vas a actualizarlo dos o tres veces durante la sesión (cierre de PR, aprendizaje, lección incidente, etc.), espera al final y junta las ediciones en un solo commit ROADMAP final.

...
**Última actualización:** 2026-06-02 (**visibilidad de borradores blindada en TODOS los vectores**). El gate "borrador no sale en prod" sólo cubría página de caso, home, `/casos`, `/graficas` y `/conexiones`; las fichas de persona/organización, el feed, los índices, `/cifras`, `/delitos`, las OG y los auto-enlaces de `RichProse` exponían contenido de casos en borrador (rol + delitos + bio + cifras + hitos recientes). Cerrado con helper central [`src/lib/visibilidad.ts`](src/lib/visibilidad.ts) + filtrado en cada vector; `/cifras` y la home cuentan sólo lo visible; `/casos` ya no pinta filas grises pero lista en **texto plano** los nombres en preparación. Build prod: **8 casos, 90 personas, 82 orgs**; barrido `dist/` **0 fugas + 0 enlaces colgantes**; `validate` 1359 OK. Ficha: [`visibilidad-estados-publicacion.md`](docs/web/features/visibilidad-estados-publicacion.md). **Commiteado; `git push` pendiente** (lo decide el maintainer).

**Anterior (2026-06-02 / 05-31).** Fugas de comentarios internos `#` en YAML cerradas (regla `V-26`; detalle en "Fugas de comentarios internos" abajo). Antes: sistema de promoción de estados ([`/promover-caso`](.agents/skills/promover-caso/SKILL.md), panel de 3 Sonnet ciegos) + `gurtel`→beta + saneo lote noos/malaya/palau/tarjetas-black (`noos` VERDE) + tanda masiva de 12 casos nuevos (`093e9ce`). Detalle: [`historial-2026-05.md`](docs/roadmap/historial-2026-05.md).

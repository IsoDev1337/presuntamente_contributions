...
**Última actualización:** 2026-06-04 (**regla 7 de afectación + `leire-diez` al día**). (1) Nueva **regla 7** del [doc 08](docs/diseno/08-afectacion-directa-indirecta.md): partido cuya caja/financiación es objeto del procedimiento → afectación **directa** (naturaleza nueva `caja_partido_objeto_investigacion_en_caso`). Schema + UI (home muestra partidos directos; orgPrincipal salta partidos) + 5 casos: PSOE (leire, filesa), PP (bárcenas-caja-b, gürtel), CDC (palau-musica); púnica y pujol revisados→siguen indirecta; 3% diferido (stub). (2) `leire-diez`: hito del **levantamiento parcial del secreto** (auto Pedraz 1-jun) + nota CGPJ N1 + delitos ampliados + auditoría editorial (empresaria anonimizada V-17, `"trama"`→`"estructura"`, `beneficiario`→`activo` en Hirurok). (3) Corregido bug de `scripts/archivar-n4.mjs` (`url_canonica` plegado rompía YAML; 3 fichas caja-b reparadas). `validate` 1389 · `check` 0/0 · `build` OK. Detalle: [`NOTES` leire](content/casos/leire-diez/NOTES.md) · [doc 08 §"Aplicación regla 7"](docs/diseno/08-afectacion-directa-indirecta.md#aplicación-de-la-regla-7-2026-06-04).
...
| Leire Díez | publicable · pin#2 | secreto levantado parcial 1-jun (hito + CGPJ N1 + PSOE afectación **directa** por regla 7); primarios cuando suba el sumario a la nube (~días); resolución de la inhibición JI 9→AN y del recurso de nulidad de Díez; auto JI 9 con número |

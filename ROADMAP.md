...
**Última actualización:** 2026-06-03 (**filtro de casos en `/graficas` + fuga de prosa en `punica`**). Nueva interacción en la sección «Cifras económicas»: ocultar casos —con la **×** de cada gráfica o un **selector de casos en el header** (estado global de la sección)— reescala el ranking «Por caso» y **recalcula en cliente** el total y las barras de grado/nivel desde un desglose pre-agregado en build; el treemap se queda completo con una nota. Los controles del header se **colapsan en móvil** en un desplegable «Filtros» con preview. Además, saneada una fuga de prosa interna (jerga de sesión) en `estado_ficha.notas` de `punica`, que se renderizaba en público. `validate` 1382 OK; build 317 págs. Detalle: [`importe-presunto.md`](docs/web/features/importe-presunto.md) + [`visualizaciones-graficas.md`](docs/web/features/visualizaciones-graficas.md). **`git push` pendiente** (16 commits por delante).

**Anterior (2026-06-03).** (a) **8 borradores cerrados** vía panel `/promover-caso`: **6 → `beta_publica`** (`barcenas-caja-b`, `atico-estepona`, `filesa`, `punica`, `forum-filatelico`, `pujol`; autorizados); `eres-andalucia`/`tandem` quedan en borrador a propósito (firmeza no limpia). (b) **API de datos abiertos**: diseño documentado **sin código** (canon [`docs/api/`](docs/api/README.md)). Antes: `V-26`, sistema `/promover-caso`, `gurtel`→beta y tanda de 12 casos. Detalle: [`historial-2026-06.md`](docs/roadmap/historial-2026-06.md) · [`historial-2026-05.md`](docs/roadmap/historial-2026-05.md).
...
- **Próximo paso:** **(1)** `git push` (lo decide el maintainer; `main` **16 commits** por delante, listo para pushear). **(2)** `eres`/`tandem`: revisar firmeza (ERE→resolución del TJUE; Tándem→casación TS) antes de panelar. **(3)** Pujol: localizar el escrito de acusación íntegro (detalle cargos/penas, hoy N4) y la sentencia (~jul-2026); `archive:catchup` de los N4 nuevos. **(4)** Zapatero 17-18 jun.

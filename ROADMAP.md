...
2. **Iconografía funcional en `badge--cat`** — `CategoryBadge.astro` nuevo, wrapper único con 11 iconos Lucide inlineados como SVG (`gavel`, `scale`, `landmark`, `building-2`, `newspaper`, `users`, `shield`, `briefcase`, `banknote`, `flag`, `radio`). Mapeo `tipo → icono` por familia: el icono identifica la familia, no la sub-especialidad (todos los órganos judiciales superiores comparten `gavel`). Refactorizado en `Hito.astro`, `CoberturaMediaticaTable.astro`, `PgOrganizaciones.astro`, `PgDelitos.astro`. `NaturalezaBadge` mantiene paleta propia sin iconos. `lucide-static@1.16.0` como devDep, no runtime. Detalle en [`docs/web/features/iconografia-badge-cat.md`](docs/web/features/iconografia-badge-cat.md).
...
**Anterior (2026-05-26 noche):** **Cloudflare Pages conectado al repo y sirviendo el preview en [`presuntamente.pages.dev`](https://presuntamente.pages.dev)** con `X-Robots-Tag: noindex` durante la fase sin DNS apex y Web Analytics activado a nivel de proyecto. Auto-deploy por push a `main` operativo. Detalle, configuración del panel y aprendizajes (primer build falló por `npm build` sin `run`; UI nueva empuja al flujo de Workers Static Assets; Web Analytics de zona DNS ≠ Web Analytics del proyecto Pages) en [`docs/web/features/cloudflare-pages-deploy.md`](docs/web/features/cloudflare-pages-deploy.md).

**Antes (tarde-noche):** cierre del sprint extendido. Sesión grande con siete piezas:
...
- [ ] **Refactor de afectación directa/indirecta** (estructural). Decidido 2026-05-27 tras detectar que la columna "Organización afectada" de `/casos` y el bloque "Partidos afectados" de la ficha pintan como "afectada" a quien sólo tiene papel procesal (acusación popular, nombramiento histórico). Taxonomía nueva (directa · indirecta · no afectada) + 6 reglas editoriales acordadas. Toca schema (`VinculoInstitucional.nivel_afectacion` + retirada de `Caso.partidos_afectados[]`), datos (recalificación de los 6 publicables), UI (`/casos` columna fusionada, `PgCasoDetalle` reformulado) y skill `/documentar-vinculos`. Sesión dedicada. Plan completo en [`docs/web/features/afectacion-directa-indirecta.md`](docs/web/features/afectacion-directa-indirecta.md).
...
- [x] **Taxonomía de afectación directa/indirecta.** Cerrada 2026-05-27 (decisión editorial; implementación en sesión dedicada): directa = sujeto procesal pasivo o víctima oficial · indirecta = institución salpicada por dependencia política/familiar/de cargo · no afectada = papel procesal (acusación popular, defensa, instructor, fiscalía, peritos). 6 reglas aplicables a fronteras (gobierno responsable, pareja sentimental, esposo/a, ministerio cuyo titular ordena el acto, acusación popular constituida, nombramiento histórico). Plan en [`docs/web/features/afectacion-directa-indirecta.md`](docs/web/features/afectacion-directa-indirecta.md).

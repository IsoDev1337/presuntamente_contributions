...
**Última actualización:** 2026-06-06 (**barrido de actualidad `/actualizar-caso`: leire-diez + plus-ultra**). **leire-diez:** 1 hito nuevo `auto-pedraz-rechazo-personacion-garcia-castellon-2026-06-04` (`auto_diligencias`, N4 ×3 líneas; Pedraz rechaza "por ahora" la personación del magistrado jubilado García-Castellón como acusación particular, ligado a la inhibición pendiente al JI nº 9) + cascada en `caso.yaml`. **plus-ultra:** ventana tranquila, **sin hito** (próximo: declaración Zapatero 17-18 jun); solo refresco de fechas + NOTES. No modelado por guardarraíles: recurso de Cerdán, Fiscalía pide a Narbona testigo, libretas UCO y **pieza de contratos SEPI/Tubos Reunidos secreta hasta julio** (donde vive el nexo fuerte con plus-ultra). `validate` 1392 OK + build limpio; **`/revisar-caso leire-diez`: 0 bloqueantes**. Detalle: [`historial-2026-06.md`](docs/roadmap/historial-2026-06.md). **Sin pushear** (working tree commiteado).

**Anterior (2026-06-04).** Coherencia web↔API en hechos retractados (`PgCasoDetalle` deja de renderizar `retirado`/`retirado_*`, conserva `superado`) + limpieza de la jerga interna «ver NOTES.md» de campos públicos (5 `estado_ficha.notas` + 19 `rol.notas` + docs/cobertura). Antes en junio: **API de datos abiertos `/api/v1/`** (CIF 1→73, `vitest` en CI) y explorador `/conexiones` (`cose`→`fcose`). Detalle: [`historial-2026-06.md`](docs/roadmap/historial-2026-06.md).

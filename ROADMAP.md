...
**Última actualización:** 2026-06-03 (**cierre de los 8 borradores restantes**). Panel `/promover-caso` (18 evaluadores Sonnet ciegos) + re-panel (12) sobre los 5 paneleables + `pujol`: **6 → `beta_publica`** (`barcenas-caja-b`, `atico-estepona`, `filesa`, `punica`, `forum-filatelico`, `pujol`; autorizados por el maintainer). `eres-andalucia` y `tandem` se quedan en borrador **a propósito** (firmeza verificada no limpia: ERE suspendido esperando al TJUE; Tándem todo recurrible). Antes del panel: resueltas las **4 incertidumbres de `pujol`** (fecha_inicio→apertura 15-jun-2021, cohecho retirado, cuadro procesal completo con Gironès + 9 empresarios, fecha hito apertura) y la sugerencia de `malaya` (nivel_fuente 1→4). El panel cazó y se arreglaron **bugs reales** (ref rota + fase en punica, error factual del TC en un rol de filesa, V-13 en forum). `validate` 1382 OK; build limpio; 0 fugas. Detalle: [`historial-2026-06.md`](docs/roadmap/historial-2026-06.md). **`git push` pendiente** (lo decide el maintainer).

**Anterior (2026-06-02).** Visibilidad de borradores blindada en todos los vectores. El gate "borrador no sale en prod" sólo cubría página de caso, home, `/casos`, `/graficas` y `/conexiones`; las fichas de persona/organización, el feed, los índices, `/cifras`, `/delitos`, las OG y los auto-enlaces de `RichProse` exponían contenido de casos en borrador (rol + delitos + bio + cifras + hitos recientes). Cerrado con helper central [`src/lib/visibilidad.ts`](src/lib/visibilidad.ts) + filtrado en cada vector; `/cifras` y la home cuentan sólo lo visible; `/casos` ya no pinta filas grises pero lista en **texto plano** los nombres en preparación. Build prod: **8 casos, 90 personas, 82 orgs**; barrido `dist/` **0 fugas + 0 enlaces colgantes**; `validate` 1359 OK. Ficha: [`visibilidad-estados-publicacion.md`](docs/web/features/visibilidad-estados-publicacion.md). En la misma sesión, **lote 1 de promoción** (`/promover-caso`, panel 3/3 VERDE + autorización): `noos`, `malaya`, `palau-musica` y `tarjetas-black` → `beta_publica`; y **roles de Pujol** actualizados (7 hijos→`acusado`, Pujol Soley→`desimputado`; sigue borrador). **Commiteado en 4 commits; `git push` pendiente** (lo decide el maintainer).

**Más atrás (05-31).** Regla `V-26` (fugas de comentarios `#` en YAML), sistema [`/promover-caso`](.agents/skills/promover-caso/SKILL.md), `gurtel`→beta y tanda de 12 casos nuevos. Detalle: [`historial-2026-05.md`](docs/roadmap/historial-2026-05.md).
...
- **Próximo paso:** **(1)** `git push` (lo decide el maintainer; `main` 11+ commits por delante). **(2)** `eres`/`tandem`: revisar firmeza (ERE→resolución del TJUE; Tándem→casación TS) antes de panelar. **(3)** Pujol: localizar el escrito de acusación íntegro (detalle cargos/penas, hoy N4) y la sentencia (~jul-2026); `archive:catchup` de los N4 nuevos. **(4)** Zapatero 17-18 jun.

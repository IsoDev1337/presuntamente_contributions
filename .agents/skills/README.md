...
- **`multi-agent-orchestration`** v0 — protocolo agnóstico de coordinación para sesiones de agentes en paralelo sobre el mismo repo git. Define worktree obligatorio para aislamiento, capa de coordinación compartida en `.git/agents/` (manifiestos de sesión + mailbox entre agentes con estado de leído), modelo pull en checkpoints, y reglas de calidad sobre qué merece la pena escribir. Incluye hook `UserPromptSubmit` de referencia (`.agents/skills/multi-agent-orchestration/inbox-check-hook.py`, Python stdlib) que notifica mensajes nuevos al inicio de cada turn; el mismo script funciona en **Claude Code** y **Codex CLI** porque ambos comparten el contrato de `UserPromptSubmit`. Activación per-user vía `.claude/settings.local.json` o `.codex/hooks.json`. Documentación íntegramente en inglés porque la skill es deliberadamente agnóstica del proyecto y reutilizable fuera de este repo. User-invocable con `/multi-agent-orchestration <acción>`.

...
5. **No clasificar orientación editorial del medio en este corpus.** La clasificación de medios vive en `Organizacion.naturaleza_editorial` + `orientacion_editorial_declarada` + `orientacion_editorial_percibida` + `grupo_editorial` (canon en [`docs/diseno/07-clasificacion-editorial-medios.md`](../../../docs/diseno/07-clasificacion-editorial-medios.md), decisión 2026-05-26). Esta skill **no** rellena esos campos al rastrear: sólo registra `medio_id`. Si al crear un medio nuevo durante el rastreo se conoce su naturaleza editorial obvia (verificación, especializado jurídico, servicio público), poblar `naturaleza_editorial` es admisible; la orientación en el eje siempre fuera del scope de esta skill.

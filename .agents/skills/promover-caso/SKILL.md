...
**2026-06-03 · cierre de los 8 borradores restantes (panel + re-panel) · 30 sub-agentes Sonnet.**
- **Panel sobre 6 casos** (los 5 paneleables + `pujol`, con sus incertidumbres ya resueltas): `barcenas-caja-b` y `atico-estepona` **VERDE 3/3**; `punica`, `filesa`, `forum-filatelico`, `pujol` **ROJO**, todos true positives:
  - `punica`: doc `nota-cgpj-condena-filtracion-punica-2017-12-04` referenciado por 2 hitos pero inexistente (V-12); `fase_actual: juicio_oral` incoherente con el último hito (`fase_intermedia`).
  - `filesa`: el rol `sala-griso-condenado-firme` decía que el TC "estimó parcialmente" el amparo cuando la STC 124/2001 lo desestimó íntegramente (contradecía su propio doc N1 + NOTES; se renderiza en público).
  - `forum-filatelico`: hecho `investigado` con una sola fuente N4 (V-13).
  - `pujol`: voto dividido (1/1/1) sobre dos docs N1 sin `url_canonica`.
- **Arreglos** (investigación web delegada a sub-agentes + verificación propia de URLs con `WebFetch`): creados los 2 docs N1 de punica con URL oficial verificada; añadida 2ª línea editorial (Confilegal) al hecho de forum; corregido el rol de filesa; corregida la fase de punica; dotados de URL/fecha verificadas los docs de pujol (la nota CGPJ de la apertura **confirma desde N1** el cuadro de delitos y la ausencia de cohecho).
- **Re-panel de los 4: VERDE 3/3 los cuatro.** El maintainer autorizó subir los **6 a `beta_publica`** (incluido `pujol` tras confirmar que tiene N1 para el armazón procesal).
- **`eres-andalucia` y `tandem` excluidos a propósito**: verificación de firmeza (junio 2026) → ERE suspendido esperando cuestión prejudicial al TJUE (sin sentencia nueva); Tándem sin condena firme (todo recurrible). Se quedan en borrador.
- **Refinamiento del rubro → A3** (aplicado arriba): un `Documento` N1 `acceso_restringido_pero_citable` sin URL es legítimo para Puerta A; no es fallo de A3. Falso positivo cazado en la 1ª pasada de `pujol`; sin la aclaración, el panel arrastra B2 a A.
- **Falsos negativos:** ninguno. **Coste:** ~1,37M tokens (panel, 18 agentes) + ~0,93M (re-panel, 12 agentes).

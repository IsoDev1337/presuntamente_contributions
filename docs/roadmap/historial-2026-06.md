...
## 2026-06-04 — Coherencia web↔API en hechos retractados + limpieza «ver NOTES.md»

Dos sugerencias heredadas de otra sesión, verificadas como ciertas antes de actuar:

- **Hecho retractado que se publicaba.** [`PgCasoDetalle.astro`](../../src/components/pages/PgCasoDetalle.astro) construía la lista de hechos filtrando sólo por `caso_id`, sin mirar `estado_publicacion`/`vigencia` → un `Hecho` retractado se renderizaba (lo confirmaba [D12](../api/decisiones.md#d12--decisiones-de-implementación-junio-2026), que lo dejó como follow-up). El único hecho retractado del repo era un **placeholder obsoleto** de `tarjetas-black` cuyo enunciado literal «Fichero obsoleto; ver …» salía en la ficha y en `/graficas`. Fix: filtro en `PgCasoDetalle` espejo de la API (excluye `vigencia: retirado` / `estado_publicacion: retirado_*`; **conserva `superado`**) + borrado del placeholder. Verificado: `Fichero obsoleto` → 0 en `dist`; hecho canónico y `superado` intactos. Canon: [`visibilidad-estados-publicacion.md`](../web/features/visibilidad-estados-publicacion.md).
- **Jerga interna «ver NOTES.md» en campos renderizados.** Más amplio de lo reportado (6 `estado_ficha.notas` eran sólo la punta): también 19 `rol.notas` (Calama + 17 roles de Pujol, que salían en ficha y en `/conexiones`) y, por la misma vía, la API JSON. Limpiados además 3 `documento.notas` y 2 corpus de cobertura (begona, **punica**) por higiene aunque no rendericen. Se reescribió cada frase para que el texto público sea autónomo, sin perder el matiz (p. ej. «pendiente de cotejo con el escrito íntegro»); la coletilla interna desaparece. `begona-gomez`/`plus-ultra` en `caso.yaml` eran sólo comentarios `#` (no fugaban). Verificado: `grep -r NOTES.md dist/` = **0**. `validate` 0 err + `build` OK.

## 2026-06-06 — Barrido de actualidad `/actualizar-caso`: leire-diez + plus-ultra

Pasada de actualidad sobre los dos casos pin del home (`/actualizar-caso leire y plus ultra`), seguida de validación mecánica (`pnpm validate` + `build`) y auditoría editorial cualitativa (`/revisar-caso leire-diez`).

- **leire-diez (ventana 3-6 jun):** 1 **Hito nuevo** `auto-pedraz-rechazo-personacion-garcia-castellon-2026-06-04` (`auto_diligencias`, N4, `hechos_introducidos: []`): el 4-jun Pedraz rechaza "por el momento" la personación como acusación particular del magistrado jubilado **Manuel García-Castellón** (alegaba ser perjudicado), porque los hechos que invoca se investigan en el JI nº 9 de Madrid y **no están aún acumulados** a la AN — enlaza con la petición de inhibición de Pedraz al JI nº 9. **V-13 holgado:** 3 N4 de líneas distintas (Confilegal jurídico-independiente + Público izq. + The Objective der.). García-Castellón ya existía como persona (instructor jubilado de Koldo) y NO se ficha rol (personación rechazada). Cascada en `caso.yaml`: frase en `estado_actual` + fechas → 2026-06-06.
- **plus-ultra (ventana 30 may-6 jun):** **sin Hito nuevo** — ventana procesalmente tranquila; el próximo hito es la declaración de Zapatero (17-18 jun). Solo refresco de fechas de revisión + NOTES. Las 4 órdenes internacionales (Reyes Rojas, López López, Baca Arbulu, Verhoeven) y el nexo Apamate/Koldo son de 25-26 may (anteriores al barrido del 30-may) y siguen pendientes de primario.
- **NO modelado (guardarraíles):** recurso de Cerdán contra la inhibición y petición de la Fiscalía de declaración testifical de Cristina Narbona (escritos/solicitudes no resueltos); contenido del sumario ya parcialmente público (libretas UCO, Mercedes González — interpretación periodística, personas sin rol formal); **pieza de contratos SEPI/Tubos Reunidos secreta hasta julio** (es donde vive el nexo fuerte con Plus Ultra: rescate PU como presunta primera actuación de Hirurok). Anotado bidireccionalmente en ambos NOTES.
- **Relación `leire-diez ↔ plus-ultra`:** reforzada en cobertura pero en pieza secreta; **no se toca**. Propuesta para ~julio: enriquecerla y valorar elevar `conexion_factual` → `comparte_actor_con` (pivote Fernández Guerrero + SEPI).
- **Validación:** `pnpm validate` 1392 OK; `build` limpio (318 páginas). **`/revisar-caso leire-diez`: 0 bloqueantes, 0 sugerencias** (CH1/CH8 sin coincidencias en prosa publicable; García-Castellón `es_figura_publica`; medios con `naturaleza_editorial`). Sin propuesta de promoción (sigue `beta_publica`; `estado_ficha` aún parcial).
- **Pendiente:** `archive:catchup -- --caso=leire-diez` para los 3 N4 nuevos (requiere red); vigilar resolución de la inhibición y levantamiento de la pieza SEPI (~julio).

## 2026-06-12 — Primer contribuyente externo (issues #3/#4) + workflow post-MVP

- **Issue #3 (propuesta editorial externa, "Contenido considerado y no modelado").** El hallazgo es real: el trabajo editorial de decidir qué NO se modela (referencias indirectas del sumario, relaciones descartadas) vive en `NOTES.md` y el lector del sitio no lo ve; ese silencio no es neutro. Se acepta la idea con la ejecución cambiada, respondido en la issue con 4 condiciones: **(1)** prosa atribuida, nunca tabla «referencia = persona» (la rejilla hace que el sitio firme la identificación, por mucho disclaimer); **(2)** sólo cargos públicos en su función pública (presidente, ministra, directora GC sí; particulares y semi-públicos no — para esos sigue el silencio); **(3)** cruce de líneas editoriales obligatorio (si sólo lo dice una trinchera, no se menciona — defensa mecánica del «sin cuota política»); **(4)** sin entidad, sin nodo, sin badge: el nombre sólo vive en esa prosa, no se crea `Persona`, no entra en `/conexiones`; `RolEnCaso` sigue siendo la única puerta al modelo de datos. **Pendiente:** formalizar como regla editorial en doc 02/04 y pilotar en la ficha de `leire-diez` (caso natural: referencias «el presidente»/«jefe» de los informes UCO). Issue retitulada + etiqueta `enhancement`.
- **Issue #4 (desgajada de la #3):** completar el catálogo editorial de medios — faltan El Mundo, ABC, La Razón, La Vanguardia, RTVE, Onda Cero y locales de casos fichados; `orientacion_editorial_percibida` poblada sólo en 5/32 (4 centroizquierda + COPE derecha, todas `fuente: cis`). Etiquetas `good first issue` + `help wanted` + `documentation`; ofrecida al contribuyente como punto de entrada.
- **Workflow post-MVP** (el sitio lleva semanas público; el «mientras dure el MVP» había quedado obsoleto): `AGENTS.md` revisado — el maintainer sigue trabajando directo a `main` con sus agentes; los **contribuyentes externos entran por rama + PR** (descriptivo con fuentes, CI verde, merge sólo del maintainer); nota explícita de que con deploy automático, push = publicar. `CONTRIBUTING.md`: la sección «futuro» pasa a «Contribuir por Pull Request» con el protocolo real y entrada por `good first issue`. **No tocado a propósito:** doc 04, apartado 12 — «no aceptar donaciones en MVP» y «no responder consultas privadas de periodistas en MVP» son decisiones de política a re-decidir por el maintainer, no limpieza de redacción (añadido como próximo paso en el roadmap).

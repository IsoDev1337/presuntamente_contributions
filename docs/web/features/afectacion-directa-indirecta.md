...
**Actualización 2026-06-04 — regla 7 (caja del partido = objeto → directa).** Añadida una séptima regla al canon ([doc 08 — "Regla 7"](../../diseno/08-afectacion-directa-indirecta.md#regla-7--partido-cuya-caja-o-financiación-es-objeto-del-procedimiento--directa)) y una naturaleza nueva `caja_partido_objeto_investigacion_en_caso` (`nivel_afectacion: directa`) al schema y a `content.config.ts`. Aplicada a 5 casos (leire-diez, barcenas-caja-b, gürtel, filesa, palau-musica); púnica y pujol revisados y mantenidos indirecta; tres-por-ciento-cataluna diferido (stub). **Cambio de UI:** el home pasa a mostrar como "Partidos afectados" los partidos afectados en **cualquier** nivel (antes solo indirecta) y la "organización principal" del preview salta los partidos (`src/lib/afectacion.ts` → `partidosAfectadosDeCaso`; `NATURALEZAS_DIRECTA` incluye la naturaleza nueva; etiquetas en `labels.ts`, `PgCasoDetalle.astro`). `validate` 1389 OK, `check` 0/0, `build` limpio. Detalle de qué caso y por qué en [doc 08 — "Aplicación de la regla 7"](../../diseno/08-afectacion-directa-indirecta.md#aplicación-de-la-regla-7-2026-06-04).

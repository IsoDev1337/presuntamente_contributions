...
- **Naturaleza**: enum cerrado de 17 valores cubriendo cargo público electo/designado/judicial, cargo orgánico de partido, directivos públicos/privados, académico público, organización cívica, nombramiento por gobierno concreto, los cinco roles institucionales en caso (acusación popular, perjudicado, entidad investigada como persona jurídica, ámbito administrativo directo del acto y afectación indirecta — las dos últimas añadidas en el refactor 2026-05-27), y los tres tipos de vínculo no procesal documentado (familiar público, económico, profesional).
- **Afectación editorial (refactor 2026-05-27)**: cuatro naturalezas marcan a una organización como afectada por el caso — `entidad_investigada_en_caso`, `perjudicado_institucional_en_caso`, `ambito_administrativo_directo_del_acto_en_caso` y `afectacion_indirecta_en_caso`. Exigen `nivel_afectacion: directa | indirecta` + `justificacion_afectacion`. La acusación popular constituida (`acusacion_institucional_en_caso`) NO es afectación: aparece en participación procesal. Canon en [`docs/diseno/08-afectacion-directa-indirecta.md`](../../diseno/08-afectacion-directa-indirecta.md). Reglas V-22..V-24 en el schema garantizan coherencia.

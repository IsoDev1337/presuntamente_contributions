...
Estructura final de la página (`PgAportar.astro`, seis secciones numeradas con `SectionH`):

1. **Qué puedes aportar** — tres tarjetas en `.carriles` (pista a fuente o hito · corrección fáctica menor · idea o sugerencia), cada una con bloque "Qué pedimos" diferenciado.
...
Cierre con `.aviso.aviso--accent` recordando que el procedimiento es el mismo independientemente del cargo del aportante, prioridad por orden de entrada + fuente firme.
...
- [ ] Cross-link recíproco desde `/rectificar` hacia `/aportar` ("¿buscabas el cauce editorial? pasa por `/aportar`"). Pendiente: requiere editar `PgRectificar.astro` en sesión siguiente.

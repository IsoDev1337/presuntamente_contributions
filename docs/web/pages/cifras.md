...
- **§1 Estado procesal de las personas** — 4 KPIs (imputados activos · condenados firme · desimputados vigentes · personas multi-caso) + distribución de asignaciones de rol con `RolBadge`.
- **§2 Distribución de casos** — por fase con `PhaseBadge` + ranking de casos por densidad documental (hitos / implicados activos / documentos), ordenado por hitos ↓.
- **§3 Documentos y fuentes** — 4 KPIs (total · primarios descargados con SHA-256 · cobertura archive.org · % N4 archivado) + distribuciones por nivel (con `LevelBadge`) y por tipo.
- **§4 Hechos y carga editorial** — grid 2-col: por estado epistémico (con `EpistemicBadge`) y por vigencia (incluye conteo de `corregido_por`).
- **§5 Hitos del inventario** — 4 KPIs (total · últimos 12 meses · jurisdiccionales · políticos+mediáticos) + último hito linkado a la ficha + top 10 tipos.
- **§6 Organizaciones citadas** — por tipo.
- **§7 Delitos catalogados y atribuidos** — por familia + top 8 atribuciones en roles (linkado a `/delitos/<slug>`).
- **§8 Conexiones y referencias auxiliares** — total de `RelacionEntreCasos` + total de entradas de glosario.
- **§9 Notas metodológicas** — definiciones operativas, recordatorio de presunción de inocencia, link a `/sobre §4` y `§5`.
...
- **Top personas por importe presuntamente atribuido en causa abierta** — ranking de personas con rol procesal activo ordenadas por importe asociado a los Hechos de los casos en que figuran. Sólo personas con rol formal (principio §5).
...
1. **Lenguaje. Principio §4 (presunción de inocencia en el lenguaje).** "Dinero robado" o "top personas que más han robado" son frases prohibidas para hechos no acreditados. La versión éticamente compatible es más fea pero correcta: *"importe presuntamente desviado según el escrito de acusación"*, *"personas con mayor importe atribuido en procedimientos abiertos"*, *"partidos cuyos miembros con rol formal acumulan mayor importe atribuido"*. Decidir el vocabulario canónico antes de implementar y testarlo con la skill `revisar-caso`. Aceptar que la versión correcta es menos pegadiza — esa es la línea ética del proyecto.

2. **Etiquetado del importe. Principio §1 (cita siempre).** Cada importe agregado debe poderse trazar al Hecho/Documento que lo afirma. Modelar `importe_presunto_perjuicio` como propiedad del `Hecho` (no del `Caso`) para que se herede el sistema de niveles de fuente N1–N4 ya existente. Un importe respaldado por sentencia firme (N1) cuenta distinto a uno respaldado sólo por una acusación popular (N3) o por cobertura periodística (N4).

3. **"Top partidos". Principio §3 (sin cuota política).** Cualquier ranking por partido será leído como sesgo por unos y como confirmación por otros, independientemente de la metodología. Tres mitigaciones a aplicar conjuntamente:
...
- **Ranking de personas más expuestas mediáticamente** — top de personas por número de Hitos en los que aparecen como afectadas. Tensión con principio §5 (no exposición innecesaria de personas privadas): solo personas con rol formal procesal.
...
- **Notas metodológicas en §9** — bajan el riesgo de citas descontextualizadas de las cifras (un periodista podría leer "27 investigados activos" y escribir "hay 27 corruptos" — el §9 corta esa lectura explícitamente).
